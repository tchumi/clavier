...
SUBJECTS = "MMM"
SESSIONS = 1
API_BASE_URL = "http://127.0.0.1:8000"  # Adresse de l'API FastAPI

def test_api_endpoint(root, endpoint="/record_password",df=None):
    ...
    try:
        ...
        messagebox.showinfo("Succès", f"Réponse : {result}")
        return result
    except requests.exceptions.RequestException as e:
        messagebox.showerror("Erreur API", f"Erreur lors de la requête : {e}")
        return None
    finally:
        # Masquer le sablier
        root.config(cursor="")
...
class KeyPressCaptureApp:
    def __init__(self, root):
        ...
        self.test_api_menu = Menu(self.menu, tearoff=0)
        self.menu.add_cascade(label="Test API", menu=self.test_api_menu)
        self.test_api_menu.add_command(label="Tester l'API", command=self.test_api)
    ...
    def test_api(self):
        ...
        print(df)
        result=test_api_endpoint(self.root,"/record_password_with_payload",df)
        messagebox.showinfo(
                        "Résultat", f"Résultat de la vérification : {result}"
        )
        return result
    ...
    def show_chronogram(self, hold_times, dd_times, ud_times):
        ...
        ax2.set_xlabel("Chronogramme des temps DD (Continu)")
